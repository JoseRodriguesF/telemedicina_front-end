...
export default function AgendamentoPage() {
    ...
    // Mock functions for calendar logic
    const today = new Date();
    const currentMonth = today.getMonth();
    const currentYear = today.getFullYear();

    // Generate days (current day + 30 days)
    const days = [];
    for (let i = 0; i < 31; i++) {
        const d = new Date();
                        ...
                        <div className="calendar-card">
                            <div className="calendar-header">
                                ...
                                <div className="calendar-nav">
                                    <button className="nav-btn" title="Anterior" disabled>
                                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6"></polyline></svg>
                                    </button>
                                    <button className="nav-btn" title="Próximo">
                            ...
                            <div className="calendar-grid">
                                ...
                                {days.map((d, i) => (
                                    <div
                                        key={i}
                                        className={`day ${selectedDate && isSameDay(d, selectedDate) ? 'selected' : ''} ${isSameDay(d, today) ? 'today' : ''}`}
