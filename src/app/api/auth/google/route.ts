...
export async function POST(req: Request) {
  try {
    ...
    if (process.env.NODE_ENV === 'development') console.log('[proxy] /api/auth/google received body:', body);
    const resp = await fetch(`${TARGET}/auth/google`, {
      method: 'POST',
    ...
    if (process.env.NODE_ENV === 'development' && resp.status >= 400) {
      console.warn('[proxy] /api/auth/google remote status:', resp.status, 'response:', data);
    }
