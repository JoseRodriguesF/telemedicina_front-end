...
export async function POST(req: Request) {
  try {
    ...
    if (process.env.NODE_ENV === 'development') console.log('[proxy] /api/register/google received body:', body);
    const resp = await fetch(`${TARGET}/register/google`, {
      method: 'POST',
    ...
    if (process.env.NODE_ENV === 'development' && resp.status >= 400) {
      console.warn('[proxy] /api/register/google remote status:', resp.status, 'response:', data);
    }
