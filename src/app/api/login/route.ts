...
export async function POST(req: Request) {
  try {
    ...
    if (process.env.NODE_ENV === 'development') console.log('[proxy] /api/login received body:', body);
    ...
    if (process.env.NODE_ENV === 'development' && resp.status >= 400) {
      console.warn('[proxy] /api/login remote status:', resp.status, 'response:', data);
    }
