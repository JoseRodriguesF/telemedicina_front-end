    ...
    try {
        ...
        if (!res.ok) {
            return NextResponse.json(
                contentType.includes('application/json') ? JSON.parse(text) : { error: text || 'Erro ao buscar consulta' },
                { status: res.status }
            );
        }
        ...
        console.error('[Proxy] Erro ao buscar consulta:', e);
        return NextResponse.json({ error: 'Falha ao buscar consulta' }, { status: 502 });
    }
}
